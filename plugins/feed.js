    ...
    switch (type) {
        ...
        case 'anjing':
            ...
            if (new Date - global.DATABASE._data.users[m.sender].anjinglastclaim > 600000) {
                if (global.DATABASE._data.users[m.sender].makananpet > 0) {
                    global.DATABASE._data.users[m.sender].makananpet -= 1
                    global.DATABASE._data.users[m.sender].anakanjing += 20
                    global.DATABASE._data.users[m.sender].anjinglastclaim = new Date * 1
                    conn.reply(m.chat, `Berhasil memberi makan pet ${type}`, m)
                    if (anjing > 0) {
                        let naiklvl = ((anjing * 100) - 1)
                        if (global.DATABASE._data.users[m.sender].anakanjing > naiklvl) {
                            global.DATABASE._data.users[m.sender].anjing += 1
                            global.DATABASE._data.users[m.sender].anakanjing -= (anjing * 100)
